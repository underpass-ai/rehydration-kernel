...
In addition to relationship explanation, the model supports provenance
metadata on nodes: \code{source_kind} (human, agent, projection, derived),
\code{source_agent}, and \code{observed_at}. Provenance flows through the
same path as explanation: emitted in projection events, persisted in Neo4j,
rehydrated in bundles, and surfaced in rendered context. When present, the
rendering pipeline annotates nodes with provenance brackets (e.g.,
\texttt{[source:agent agent=diagnostics observed=2026-03-25T14:00Z]}) so
that downstream consumers can assess source trust without additional queries.

Provenance is an observability feature, not an inference signal.
Benchmarking with differentiated provenance (human sources on root nodes,
agent on decisions, projection on distractors) shows no improvement in
LLM accuracy: explanatory bundles score 18/18 with or without provenance,
and the \mbox{${\sim}22\%$} token overhead from provenance brackets
slightly degrades structural scores. The value of provenance is in
auditability for human operators and trust-assessment systems, not in
...
infrastructure that keeps that explanation and provenance intact across
projection, storage, query, and rendering boundaries.
...
meso and stress graphs), Qwen3-8B with flat rendering drops from 100\% to
83\% on explanatory bundles, with \texttt{stress-ops-explanatory} falling to
0/3. The kernel addresses this through \texttt{RehydrationMode}: an
auto-detection heuristic that selects \texttt{resume\_focused} when the
tokens-per-node ratio falls below a threshold ($<$30). In this mode, the
tiered renderer prunes all distractor branches and allocates the full budget
to the causal spine (L1). Table~\ref{tab:modecomparison} shows the effect.

\begin{table}[h]
\centering\small
\begin{tabular}{lccc}
\toprule
Rendering & Expl. & Struct. & Gap \\
\midrule
Flat @512             & 15/18 (83\%) & 8/18 (44\%) & 39pp \\
Tiered + ResumeFocused @512 & 16/18 (89\%) & 5/18 (28\%) & 61pp \\
\bottomrule
\end{tabular}
\caption{Effect of \texttt{resume\_focused} mode at 512-token budget with
Qwen3-8B. The mode fixes \texttt{stress-ops-explanatory} from 0/3 to 3/3
by pruning distractors and giving the causal spine the full budget.}
\label{tab:modecomparison}
\end{table}

At full budget the local model achieves 100\% because rationale text is
present verbatim and the model extracts rather than reasons. The 89\% result
under token pressure with \texttt{resume\_focused} reflects genuine context
quality: the kernel's mode-aware rendering selects which relationships
survive truncation, and the causal spine is sufficient for task identification,
restart-point selection, and rationale preservation.

To stress-test robustness, we introduce \texttt{CompetingCausal} noise:
distractor branches that use domain vocabulary, causal semantic classes, and
plausible rationale mimicking real relationships. With competing noise,
explanatory bundles maintain 100\% (18/18) while structural drops from 50\%
to 28\% (5/18). The explanatory metadata is immune to plausible-looking
noise because the model can anchor on specific rationale text; without it,
competing causal distractors are indistinguishable from the real chain.
...
configurations, while structural-only bundles reach 61\%. A local 8B model
with explanatory context matches frontier accuracy, and a mode-aware rendering
heuristic (\texttt{resume\_focused}) recovers the worst-case configuration
from 0/3 to 3/3 under severe token pressure by pruning non-causal branches.
The next step is noise injection experiments and closed-loop evaluation where
the selected rehydration point leads to a corrected retry.
