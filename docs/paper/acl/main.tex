...
reconstruction under a \mbox{96}-token budget in the constraint-preserving
...
detail primarily improves completeness.
\end{abstract}
...
\mbox{4096} tokens, a full explanatory variant at \mbox{96} tokens, and a
structural-only variant at \mbox{96} tokens. This isolates the contribution
of explanatory relationships from the contribution of extended node detail and
tests whether those relationships survive bounded retrieval.
...
carrier of intent. The same pattern appears in UC4: at a \mbox{96}-token
...
context rendered at \mbox{96} tokens still reaches \mbox{1.0} causal
reconstruction and preserves the dominant reason, even though detail fidelity
falls to \mbox{0.0}. The structural-only variant at the same budget drops to
...
rendered context falls from 135 to 97 tokens. In UC2 it falls from 126 to 91.
In UC3 it falls from 260 to 169. In UC4 the explanatory \mbox{@96} variant
uses 89 tokens, while the structural-only \mbox{@96} variant uses 64. In all
...
The current evidence has several limitations. First, the quantitative use
cases are small and synthetic. They isolate the explanatory-variable effect
cleanly, but they do not yet demonstrate performance on larger, noisier, or
multi-branch operational graphs. Second, the measured local bundles remain
modest in size. The current artifact includes only a limited meso-scale
distractor graph for UC1, so it does not yet establish behavior across
substantially denser retrieval neighborhoods. Third, the current metrics
center on explanatory use cases rather than the full pull-driven and
event-driven suite. Fourth, runtime diversity remains limited. Fifth,
explanation quality is application-supplied; the kernel preserves typed shape
and transport, but it does not validate truthfulness or completeness of
emitted rationale. This means the present results are best interpreted as
evidence about preservation and utility of explanatory context, not about
automatic discovery of causal structure.
...
\midrule
UC1 & 0.429 \metricbar{4.3} & 0 & 113 \\
UC2 & 0.429 \metricbar{4.3} & n/a & 111 \\
UC3 & 0.429 \metricbar{4.3} & 0 & 208 \\
\bottomrule
\end{tabular}
\end{minipage}\hfill
\begin{minipage}{0.47\textwidth}
...
\midrule
UC1 micro & 1.000 \metricbar{10.0} & 1 & 135 \\
UC1 meso & 1.000 \metricbar{10.0} & 1 & 135 \\
\bottomrule
\end{tabular}
\end{minipage}
