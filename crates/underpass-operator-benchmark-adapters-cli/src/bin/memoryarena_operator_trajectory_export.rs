...
fn usage() -> String {
    "usage: memoryarena_operator_trajectory_export --run <memoryarena-run-dir> --output <dir> [--expected-run-id id] [--include-writer-reads] [--force]".to_string()
}
...
fn export_results(path: &Path, data: &mut ExportData) -> Result<(), Box<dyn Error + Send + Sync>> {
    for (line_index, value) in read_jsonl(path)?.into_iter().enumerate() {
        ...
        for (call_index, call) in calls.iter().enumerate() {
            ...
            let target_error =
                target_action_validation_error("read", &allowed_tools, &target_action);
