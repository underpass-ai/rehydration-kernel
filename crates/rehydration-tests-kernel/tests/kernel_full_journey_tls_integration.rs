...
use support::kernel_tls_fixture::KernelTlsFixture;

#[tokio::test]
async fn kernel_full_journey_supports_tls_across_query_and_command_surfaces()
-> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    let result = async {
        ...
        assert!(query_role_bundle.neighbor_nodes.iter().any(|node| node.node_id == TASK_ID));
        assert!(query_role_bundle.node_details.iter().any(|detail| detail.node_id == TASK_ID));
        assert!(query_role_bundle.neighbor_nodes.iter().any(|node| node.node_id == EXPLORER_WORKSTREAM_ID));
        assert!(query_role_bundle.neighbor_nodes.iter().any(|node| node.node_id == EXPLORER_CHECKLIST_ID));
        assert!(query_role_bundle.neighbor_nodes.iter().any(|node| node.node_id == EXPLORER_LEAF_ID));
        assert!(query_role_bundle.node_details.iter().any(|detail| detail.node_id == EXPLORER_WORKSTREAM_ID));
        assert!(query_role_bundle.node_details.iter().any(|detail| detail.node_id == EXPLORER_LEAF_ID));
        assert!(query_role_bundle.relationships.iter().any(|edge| {
            edge.source_node_id == DECISION_ID && edge.target_node_id == TASK_ID && edge.relationship_type == RELATION_IMPACTS
        }));
        assert!(query_role_bundle.relationships.iter().any(|edge| {
            edge.source_node_id == POWER_TASK_ID && edge.target_node_id == TASK_ID && edge.relationship_type == RELATION_DEPENDS_ON
        }));
        assert!(query_role_bundle.relationships.iter().any(|edge| {
            edge.source_node_id == JUMP_DECISION_ID && edge.target_node_id == DECISION_ID && edge.relationship_type == RELATION_DECISION_REQUIRES
        }));
        let rendered_query_context = query_context.rendered.expect("rendered context should exist");
        ...
        assert!(rendered_query_context.content.contains(PROPULSION_SUBSYSTEM_TITLE));
        assert!(rendered_query_context.content.contains(CHIEF_ENGINEER_TITLE));
        assert!(rendered_query_context.content.contains(EXPLORER_WORKSTREAM_TITLE));
        assert!(rendered_query_context.content.contains(EXPLORER_WORKSTREAM_DETAIL));
        assert!(rendered_query_context.content.contains(EXPLORER_CHECKLIST_TITLE));
        assert!(rendered_query_context.content.contains(EXPLORER_LEAF_TITLE));
        assert!(rendered_query_context.content.contains(EXPLORER_LEAF_DETAIL));

        let node_detail = query_client
            .get_node_detail(GetNodeDetailRequest { node_id: EXPLORER_LEAF_ID.to_string() })
            .await?.into_inner();
        assert_eq!(node_detail.node.as_ref().map(|node| node.node_id.as_str()), Some(EXPLORER_LEAF_ID));
        assert_eq!(node_detail.node.as_ref().map(|node| node.title.as_str()), Some(EXPLORER_LEAF_TITLE));
        assert_eq!(node_detail.detail.as_ref().map(|detail| detail.detail.as_str()), Some(EXPLORER_LEAF_DETAIL));
        ...
        assert!(zoomed_role_bundle.neighbor_nodes.iter().any(|node| node.node_id == EXPLORER_CHECKLIST_ID));
        assert!(zoomed_role_bundle.neighbor_nodes.iter().any(|node| node.node_id == EXPLORER_LEAF_ID));
        let zoomed_rendered = zoomed_context.rendered.expect("zoomed rendered context should exist");
        ...
        assert_eq!(query_rehydrate.snapshot_id, format!("snapshot:{ROOT_NODE_ID}:{DEVELOPER_ROLE}"));
        let query_rehydrate_bundle = query_rehydrate.bundle.expect("rehydrated bundle should exist");
        assert_eq!(query_rehydrate_bundle.root_node_id, ROOT_NODE_ID);
        let query_rehydrate_stats = query_rehydrate_bundle.stats.as_ref().expect("rehydrated stats should exist");
        assert_eq!(query_rehydrate_stats.nodes, EXPECTED_SELECTED_NODE_COUNT);
        assert_eq!(query_rehydrate_stats.relationships, EXPECTED_SELECTED_RELATIONSHIP_COUNT);
        assert_eq!(query_rehydrate_stats.detailed_nodes, EXPECTED_DETAIL_COUNT as u32);
        assert_eq!(query_rehydrate_stats.timeline_events, 11);
        assert_eq!(query_rehydrate_bundle.bundles.len(), 1);
        assert_eq!(query_rehydrate_bundle.bundles[0].neighbor_nodes.len(), EXPECTED_NEIGHBOR_COUNT);
        assert_eq!(query_rehydrate_bundle.bundles[0].relationships.len(), EXPECTED_RELATIONSHIP_COUNT);
        assert_eq!(query_rehydrate_bundle.bundles[0].node_details.len(), EXPECTED_DETAIL_COUNT);
        ...
        let leaf_bundle = leaf_rehydrate.bundle.expect("leaf rehydration bundle should exist");
        ...
        assert_eq!(leaf_role_bundle.node_details[0].detail, EXPLORER_LEAF_DETAIL);
        ...
        let command_update = command_client
            ...
            .await?.into_inner();
        assert_eq!(command_update.accepted_version.as_ref().map(|version| version.revision), Some(1));
