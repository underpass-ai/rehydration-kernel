...
use rehydration_tests_shared::metrics::emit_metric;
use rehydration_tests_shared::metrics::use_case_harness::{
...
-> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    assert!(baseline.metric.explanation_roundtrip_fidelity > structural_only.metric.explanation_roundtrip_fidelity);
    assert!(baseline.metric.causal_reconstruction_score > structural_only.metric.causal_reconstruction_score);
    assert!(baseline.metric.detail_roundtrip_fidelity > without_detail.metric.detail_roundtrip_fidelity);
    assert!(baseline.metric.causal_reconstruction_score > without_detail.metric.causal_reconstruction_score);
    assert!(detail_only.metric.causal_reconstruction_score > structural_only.metric.causal_reconstruction_score);
    assert!(baseline.metric.causal_reconstruction_score > detail_only.metric.causal_reconstruction_score);
    ...
    assert!(!structural_only.metric.rendered_contains_expected_decision_reference);
    ...
    assert!(detail_only.metric.rendered_contains_expected_decision_reference);
...
-> Result<(), Box<dyn Error + Send + Sync>> {
    let baseline = observe_why_task_was_implemented_that_way(PaperUseCaseVariant::FULL_EXPLANATORY_WITH_DETAIL).await?;
    let structural_only = observe_why_task_was_implemented_that_way(PaperUseCaseVariant::STRUCTURAL_ONLY_WITH_DETAIL).await?;
    let without_detail = observe_why_task_was_implemented_that_way(PaperUseCaseVariant::FULL_EXPLANATORY_WITHOUT_DETAIL).await?;
    let detail_only = observe_why_task_was_implemented_that_way(PaperUseCaseVariant::DETAIL_ONLY_WITH_DETAIL).await?;
...
-> Result<(), Box<dyn Error + Send + Sync>> {
    let baseline = observe_interrupted_handoff_use_case(PaperUseCaseVariant::FULL_EXPLANATORY_WITH_DETAIL).await?;
    let structural_only = observe_interrupted_handoff_use_case(PaperUseCaseVariant::STRUCTURAL_ONLY_WITH_DETAIL).await?;
    let without_detail = observe_interrupted_handoff_use_case(PaperUseCaseVariant::FULL_EXPLANATORY_WITHOUT_DETAIL).await?;
    let detail_only = observe_interrupted_handoff_use_case(PaperUseCaseVariant::DETAIL_ONLY_WITH_DETAIL).await?;
...
-> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    assert!(baseline.metric.detail_roundtrip_fidelity > low_budget.metric.detail_roundtrip_fidelity);
    assert!(low_budget.metric.causal_reconstruction_score > structural_low_budget.metric.causal_reconstruction_score);
    assert_eq!(low_budget.metric.dominant_reason_hit, Some(true));
    assert_eq!(structural_low_budget.metric.dominant_reason_hit, Some(false));
    assert_eq!(low_budget.metric.causal_reconstruction_score, 1.0);
    assert!(low_budget.metric.explanation_roundtrip_fidelity > structural_low_budget.metric.explanation_roundtrip_fidelity);
