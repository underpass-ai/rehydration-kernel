...
use rehydration_proto::fleet_context_v1::{GetContextRequest, GetContextResponse};
use tonic::{Request, Response, Status};

use crate::transport::context_service_compatibility::{
    ContextCompatibilityGrpcService, request_mapping::map_get_context_query,
    response_mapping::proto_get_context_response, status_mapping::map_compatibility_error,
};
    ...
    S: SnapshotStore + Send + Sync + 'static,
{
    let result = service
        .query_application
        .get_context(map_get_context_query(request.into_inner()))
        .await
        .map_err(map_compatibility_error)?;

    Ok(Response::new(proto_get_context_response(&result)))
}
