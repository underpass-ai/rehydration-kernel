...
use crate::support::golden_contract::{
    expected_budgeted_get_context_response, expected_focused_get_context_response,
    expected_get_context_response,
};
use crate::support::seed_data::{BUILD_PHASE, DEVELOPER_ROLE, ROOT_NODE_ID, TASK_ID};
...
#[tokio::test]
async fn grpc_get_context_subtask_focus_matches_golden_contract()
-> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    let result = async {
        ...
        assert_eq!(response, expected_focused_get_context_response());
        Ok::<(), Box<dyn Error + Send + Sync>>(())
    }
    .await;
...
#[tokio::test]
async fn grpc_get_context_respects_token_budget_hint() -> Result<(), Box<dyn Error + Send + Sync>> {
    let mut fixture = SeededCompatibilityFixture::start().await?;
    ...
    let result = async {
        ...
        assert_eq!(response, expected_budgeted_get_context_response());
        Ok::<(), Box<dyn Error + Send + Sync>>(())
    }
    .await;
