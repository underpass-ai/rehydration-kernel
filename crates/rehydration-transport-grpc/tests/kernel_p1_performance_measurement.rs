...
///   - 3× single-role RehydrateSession (old path: repeated graph reads)
///   - 1× multi-role RehydrateSession (new path: shared graph read)
///
...
-> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    let result = async {
        ...
        // ── Baseline: 3× single-role calls (simulates pre-P1 behavior) ──
        ...
        // ── Optimized: 1× multi-role call (P1 shared-read path) ──
        ...
        eprintln!("╠══════════════════════════════════════════════════════════════╣");
        eprintln!("║                                                              ║");
        eprintln!("║  BASELINE (3× single-role calls):                            ║");
        ...
        eprintln!("║                                                              ║");
        eprintln!("║  OPTIMIZED (1× multi-role call):                             ║");
        ...
        // All role bundles share the same graph — same neighbor set.
        ...
        // ── Assertion: multi-role should be faster than 3× single ──
        // Allow generous margin — the point is directional, not exact.
        assert!(
            optimized_total_ms < baseline_total_ms * 1.1,
            "multi-role ({optimized_total_ms:.2}ms) should not be slower than 3× single ({baseline_total_ms:.2}ms)"
        );
