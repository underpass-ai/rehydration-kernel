...
use crate::agentic_support::runtime_workspace::{
    AgentRuntime, RuntimeResult, ToolDescriptor, ToolInvocation,
};

pub(crate) struct UnderpassRuntimeClient {
    authority: String,
    session_id: String,
}
...
fn parse_json_response<T>(response: &str) -> Result<T, Box<dyn Error + Send + Sync>>
where
    T: for<'de> Deserialize<'de>,
{
