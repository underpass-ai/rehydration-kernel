...
use crate::agentic_support::agentic_fixture::AgenticFixture;
use crate::agentic_support::explanatory_seed_data::{
    ...
    RECOVERY_DECISION_RELATION, RECOVERY_SUCCESS_RATIONALE, ROOT_NODE_ID, ROOT_NODE_KIND,
...
pub(crate) const FAILURE_DIAGNOSIS_QUESTION: &str = "Given a graph that implemented a task incorrectly, can we isolate the suspect relationships and identify the upstream rehydration point?";
pub(crate) const WHY_IMPLEMENTED_USE_CASE_ID: &str = "uc2_why_implementation_trace";
pub(crate) const WHY_IMPLEMENTED_TITLE: &str = "Why-was-this-implemented-like-this analysis";
pub(crate) const WHY_IMPLEMENTED_QUESTION: &str = "Given a task that was implemented in a specific way, can we reconstruct the reason for that implementation choice from graph context alone?";
pub(crate) const HANDOFF_RESUME_USE_CASE_ID: &str = "uc3_interrupted_handoff_resume";
pub(crate) const HANDOFF_RESUME_TITLE: &str = "Interrupted handoff and resumable execution";
pub(crate) const HANDOFF_RESUME_QUESTION: &str = "Given a task interrupted after a failed implementation step, can the system recover why it was handed off and from which upstream node execution should resume?";
pub(crate) const CONSTRAINT_PRESSURE_USE_CASE_ID: &str =
    "uc4_constraint_reason_under_token_pressure";
pub(crate) const CONSTRAINT_PRESSURE_TITLE: &str =
    "Constraint-preserving retrieval under token pressure";
pub(crate) const CONSTRAINT_PRESSURE_QUESTION: &str = "Given a task chosen under a binding safety constraint, can the system preserve the dominant reason even when the rendered context is strongly budgeted?";

#[derive(Debug, Clone, Copy)]
pub(crate) struct PaperUseCaseVariant {
    pub(crate) variant_id: &'static str,
    pub(crate) variant_label: &'static str,
    pub(crate) seed_variant: ProjectionSeedVariant,
}
...
impl PaperUseCaseVariant {
    ...
    pub(crate) fn relation_variant_label(self) -> &'static str {
    ...
    pub(crate) fn detail_variant_label(self) -> &'static str {
    ...
    pub(crate) fn graph_scale_label(self) -> &'static str {
    ...
    pub(crate) fn metric_variant_id(self, token_budget: u32) -> String {
    ...
    pub(crate) fn metric_variant_label(self, token_budget: u32) -> String {
...
pub(crate) struct FailureDiagnosisObservation {
    pub(crate) metric: PaperUseCaseMetric,
    pub(crate) suspect_relationships: Vec<PaperMetricRelationship>,
    pub(crate) rehydration_node_id: String,
    pub(crate) rendered_content: String,
}

pub(crate) struct ImplementationWhyObservation {
    pub(crate) metric: PaperUseCaseMetric,
    pub(crate) rationale: String,
    pub(crate) motivation: String,
    pub(crate) decision_id: String,
    pub(crate) caused_by_node_id: String,
    pub(crate) rendered_content: String,
}

pub(crate) struct HandoffResumeObservation {
    pub(crate) metric: PaperUseCaseMetric,
    pub(crate) continuation_node_id: String,
    pub(crate) blocker_rationale: String,
    pub(crate) motivation: String,
    pub(crate) rendered_content: String,
}

pub(crate) struct ConstraintPressureObservation {
    pub(crate) metric: PaperUseCaseMetric,
    pub(crate) constraint_rationale: String,
    pub(crate) implementation_rationale: String,
    pub(crate) rendered_content: String,
}
...
pub(crate) async fn observe_failure_diagnosis_use_case(
    ...
    let fixture = AgenticFixture::start_with_seed_and_readiness(
        ROOT_NODE_ID,
        FAILURE_FOCUS_NODE_ID,
        variant.seed_variant.detail_mode == DetailMode::WithDetail,
        |publisher| async move {
            publish_flawed_task_projection_events_variant(&publisher, variant.seed_variant).await
        },
    )
    .await?;
...
pub(crate) async fn observe_why_task_was_implemented_that_way(
    variant: PaperUseCaseVariant,
    ...
    let fixture = AgenticFixture::start_with_seed_and_readiness(
        ...
        variant.seed_variant.detail_mode == DetailMode::WithDetail,
        |publisher| async move {
            publish_explanatory_projection_events_variant(&publisher, variant.seed_variant).await
        },
    )
    .await?;
...
pub(crate) async fn observe_interrupted_handoff_use_case(
    variant: PaperUseCaseVariant,
) -> Result<HandoffResumeObservation, Box<dyn Error + Send + Sync>> {
    let fixture = AgenticFixture::start_with_seed_and_readiness(
        HANDOFF_ROOT_NODE_ID,
        HANDOFF_RESUMED_TASK_NODE_ID,
        variant.seed_variant.detail_mode == DetailMode::WithDetail,
        |publisher| async move {
            publish_handoff_projection_events_variant(&publisher, variant.seed_variant).await
        },
    )
    .await?;
...
pub(crate) async fn observe_constraint_under_token_pressure_use_case(
    variant: PaperUseCaseVariant,
    token_budget: u32,
    ...
    let fixture = AgenticFixture::start_with_seed_and_readiness(
        ...
        variant.seed_variant.detail_mode == DetailMode::WithDetail,
        |publisher| async move {
            publish_constraint_projection_events_variant(&publisher, variant.seed_variant).await
        },
    )
    .await?;
...
pub(crate) fn contains_metric_relationship(
