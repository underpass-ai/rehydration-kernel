...
pub(crate) struct RunningGrpcServer {
...
impl RunningGrpcServer {
    pub(crate) async fn start<G, D, S>(
        graph_reader: G,
        detail_reader: D,
        snapshot_store: S,
        ...
        S: SnapshotStore + Send + Sync + 'static,
    {
        ...
        let server = GrpcServer::new(config, graph_reader, detail_reader, snapshot_store);
    ...
    pub(crate) async fn connect_channel(&self) -> Result<Channel, Box<dyn Error + Send + Sync>> {
